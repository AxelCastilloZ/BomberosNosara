...
export class EquipoBomberilSchedulerService {
  ...
  async enviarNotificacionesMantenimiento(): Promise<void> {
    this.logger.log('=== INICIANDO: Envío de notificaciones de mantenimiento de equipos ===');
    ...
    try {
      ...
      if (mantenimientos.length === 0) {
        this.logger.log('No hay mantenimientos de equipos programados para mañana.');
        return;
      }
  ...
  async cambiarEstadoMantenimientosDelDia(): Promise<void> {
    this.logger.log('=== INICIANDO: Cambio automático de estado de mantenimientos de equipos ===');
    ...
    try {
      ...
      for (const mantenimiento of mantenimientos) {
        ...
        this.logger.debug(
          `Mantenimiento ${mantenimiento.id} del equipo ${mantenimiento.equipo.nombre} → EN_REVISION`
        );
      }

      this.logger.log(`✅ ${mantenimientos.length} mantenimiento(s) de equipos cambiado(s) a EN_REVISION.`);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Error desconocido';
  ...
  private async enviarEmailNotificacion(
    ...
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background-color: #fff3cd; border: 1px solid #ffc107; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
          <h2 style="color: #856404; margin: 0;">⚠️ Recordatorio de Mantenimiento de Equipos</h2>
        </div>
        
        <p style="font-size: 16px;">
          <strong>Los siguientes equipos tienen mantenimiento programado para MAÑANA:</strong>
        </p>
    ...
    try {
      await this.mailer.sendMail({
        to: emails,
        subject: `🚒 Recordatorio: ${mantenimientos.length} mantenimiento(s) de equipos programado(s) para mañana`,
        html: htmlContent,
      });
